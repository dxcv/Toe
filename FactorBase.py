...
#             i += 1
    ...
    def portsWaDateReturnList(self, ports, date, interval, factor_col):
    ...
    def portsWaYearlyReturnList(self, port_stk_num, start_date, end_date, interval, factor_col):
        # 计算start_date, end_date 中间有多少个交易日, 那就应该包括start_date和 end_date
        total_trade_days = get_intermediate_trading_days(start_date, end_date)
        ...
        for i in range(interval_num): 
            ...
            ports_return_list.append(self.portsWaDateReturnList(ports, date, interval, factor_col))
    ...
    def factorsPortsCorrCal(self, port_stk_num, start_date, end_date, factor_col):
        ...
        if self.data4cal[factor_col].dtype == "bool":
            return 0,0
        ports_wa_yearly_return_array = np.array(self.portsWaYearlyReturnList(
            port_stk_num, start_date, end_date, self.interval, factor_col))
    ...
    def refineFactors(self, threshhold = 0.5):
        ...
        for factor in self.raw_factors:
            ...
            if self.factor_corr_dict[factor][0] > 0:
                ...
                if self.probExcessHs300(ports[0], self.start_date, self.end_date, factor) > threshhold:
                    self.refined_factors.append(factor)
            else:
                # start_date 计算出后续需要持有的股票后，在持有期间计算大因子port跑输hs300的概率(若负相关)
                if self.probBelowHs300(ports[0], self.start_date, self.end_date, factor) > threshhold:
                    self.refined_factors.append(factor)
    ...
    def getMaxReturnFactor(self, factor_list):
        ...
        for factor in factor_list:
            tmp_return = self.portsWaYearlyReturnList(self.port_stk_num, self.start_date, self.end_date, self.interval, factor)
            # 不能用tmp_return, 得用port1的return， portsWaYearlyReturnList是 [port1_return, port2_return, ...]
            if tmp_return[0] > max_return:
                max_return = tmp_return[0]
                max_factor = factor
        return max_factor            
    ...
    def corrFilteredFactors(self, threshold, raw_factors):
        ...
        # 调取raw_factor对应的 df
        corrdf = self.data4cal.reset_index().set_index("trade_date").loc[self.start_date:self.end_date, raw_factors].corr()
    ...
    def getHolding(self, date):
        # 重复调用getHolding的时候，避免再次写入 "factor4rank" 这个列
