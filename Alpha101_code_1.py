...
class Alphas(object):
    def __init__(self, df_data):
        ...
        # 修改 key, 就是 "S_DQ_OPEN"，把这个变成我们的字段就可以了。这是时间序列?
        ...
        self.vwap = (df_data['S_DQ_AMOUNT'])/(df_data['S_DQ_VOLUME']+1)
...
if __name__ == "__main__":
    from PreExecutor import PreExecutor
    df = pd.read_csv("E:/data/000001.SZ.CSV", encoding="gb18030")
    pre = PreExecutor(df)

    df = pre.modify4alpha101()
    get_alpha(df)
    print(df)
